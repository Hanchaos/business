 ...
 */
#ifndef PICSERVER_INCLUDE_PICSERVER_SERVER_HTTP_H_
#define PICSERVER_INCLUDE_PICSERVER_SERVER_HTTP_H_
...
class ServerBase {
 ...
 protected:
  class Connection : public std::enable_shared_from_this<Connection> {
   public:
    ...
    void close() noexcept {
      error_code ec;
      std::unique_lock<std::mutex> lock(socket_close_mutex);  // The following
      // operations
      // seems to be
      // needed to run
      // sequentially
 ...
 public:
  ...
  virtual void start() {
    ...
    if (internal_io_service) {
      ...
      std::cout << "service thread count: " << config.thread_pool_size
                << std::endl;
 ...
 protected:
  ...
  void write_response(
    ...
    session->connection->set_timeout(config.timeout_content);
    auto response =
        std::shared_ptr<Response>(new Response(session, config.timeout_content),
...
class Server<HTTP> : public ServerBase<HTTP> {
 ...
 protected:
  void accept() override {
    ...
    acceptor->async_accept(*session->connection->socket,
                           [this, session](const error_code &ec) {
      auto lock = session->connection->handler_runner->continue_lock();
      if (!lock) {
        session->callback(session->connection,
...
#endif  // PICSERVER_INCLUDE_PICSERVER_SERVER_HTTP_H
