...
#ifndef PICSERVER_INCLUDE_PICSERVER_CLIENT_HTTP_H_
#define PICSERVER_INCLUDE_PICSERVER_CLIENT_HTTP_H_
...
class ClientBase {
 ...
 protected:
  class Connection : public std::enable_shared_from_this<Connection> {
   public:
    ...
    std::unique_ptr<socket_type> socket;  // Socket must be unique_ptr since
 ...
 public:
  ...
  /// calling request().
  /// When using asynchronous requests, running the io_service is up to the
  /// programmer.
  std::shared_ptr<asio::io_service> io_service;

  /// Convenience function to perform synchronous request. The io_service is run
  /// within this function.
  /// If reusing the io_service for other tasks, use the asynchronous request
  /// functions instead.
  ...
  std::shared_ptr<Response> request(const std::string &method,
    ...
    request(method, path, content, header,
            [&response, &ec](std::shared_ptr<Response> response_,
  ...
  /// Asynchronous request where setting and/or running Client's io_service is
  /// required.
  ...
  void request(const std::string &method, const std::string &path,
    ...
    session->callback = [this, response, request_callback](
        const std::shared_ptr<Connection> &connection, const error_code &ec) {
      {
 ...
 protected:
  ...
  std::shared_ptr<Connection> get_connection() noexcept {
    ...
    if (!query) {
      if (config.proxy_server.empty())
        ...
        query = std::unique_ptr<asio::ip::tcp::resolver::query>(
            new asio::ip::tcp::resolver::query(
    ...
    asio::async_write(*session->connection->socket,
      ...
      if (!lock) {
        session->callback(session->connection,
                          make_error_code::make_error_code(errc::broken_pipe));
        return;
      }
      if (!ec)
        this->read(session);
  ...
  void read_chunked(const std::shared_ptr<Session> &session,
    ...
    // chunked_streambuf is needed as new read buffer with its size adjusted
    // depending on the size of tmp_streambuf
    auto chunked_streambuf = std::make_shared<asio::streambuf>(
        config.max_response_streambuf_size - tmp_streambuf->size());
    // Move excess read data from session->response->streambuf to
    // chunked_streambuf
    ...
    asio::async_read_until(*session->connection->socket, *chunked_streambuf,
      ...
      if (!ec) {
        ...
        auto num_additional_bytes =
            chunked_streambuf->size() - bytes_transferred;
        ...
        auto post_process =
          ...
          if (length > 0) {
            ...
            chunked_stream.read(buffer.get(),
...
#endif  // PICSERVER_INCLUDE_PICSERVER_CLIENT_HTTP_H_
