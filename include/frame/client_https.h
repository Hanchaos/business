...
#ifndef PICSERVER_INCLUDE_PICSERVER_CLIENT_HTTPS_H_
#define PICSERVER_INCLUDE_PICSERVER_CLIENT_HTTPS_H_
   ...
   protected:
    ...
    void connect(const std::shared_ptr <Session> &session) override {
      if (!session->connection->socket->lowest_layer().is_open()) {
        ...
        resolver->async_resolve(*query, [this, session, resolver](
            const error_code &ec,
          ...
          if (!ec) {
            ...
            asio::async_connect(
                session->connection->socket->lowest_layer(), it,
                [this, session, resolver](
                  ...
                  if (!ec) {
                    ...
                    if (!this->config.proxy_server.empty()) {
                      ...
                      auto host_port =
                          this->host + ':' + std::to_string(this->port);
                      ...
                      asio::async_write(
                          ...
                          *write_buffer, [this, session, write_buffer](
                              const error_code &ec,
    ...
    void handshake(const std::shared_ptr <Session> &session) {
      ...
      session->connection->socket->async_handshake(
          asio::ssl::stream_base::client, [this, session](const error_code &ec) {
            session->connection->cancel_timeout();
            auto lock = session->connection->handler_runner->continue_lock();
            if (!lock)  {
              session->callback(session->connection, 
...
#endif  //  PICSERVER_INCLUDE_PICSERVER_CLIENT_HTTPS_H_
